...
class PlatformApiTests {
  companion object {
    ...
    fun `test sep31 post, get and patch`() {
      ...
      //        GsonUtils.getInstance().fromJson(testCustomer1Json,
      ...
      //        GsonUtils.getInstance().fromJson(testCustomer2Json,
      // Sep12PutCustomerRequest::class.java)
      ...
      //      val txnRequest = gson.fromJson(postTxnRequest,
      // Sep31PostTransactionRequest::class.java)
    ...
    //      var updatedReceiverCustomer = sep12Client.getCustomer(receiverCustomer.id,
    // "sep31-receiver")
    ...
    //      assertEquals(getTxResponse.id, patchedTx.id)
    //      assertEquals(TransactionEvent.Status.PENDING_CUSTOMER_INFO_UPDATE.status,
    // patchedTx.status)
    ...
    //
    //      // PATCH {platformAPI}/transaction status to COMPLETED, since the clabe_number was
    // updated
    //      // correctly.
    //      patchTxRequest =
    //        PatchTransactionRequest.builder()
    ...
    //      assertEquals(postTxResponse.id, gotSep31TxResponse.transaction.id)
    //      assertEquals(TransactionEvent.Status.COMPLETED.status,
    // gotSep31TxResponse.transaction.status)
