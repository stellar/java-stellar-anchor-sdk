...
public class Sep24Service {
    ...
    public Sep24Service(AppConfig appConfig,
    ...
    public InteractiveTransactionResponse withdraw(String fullRequestUrl, JwtToken token, Map<String, String> withdrawRequest) throws SepException, MalformedURLException, URISyntaxException {
        if (token == null) {
        ...
        Log.infoF("Sep24.withdraw. account={}, memo={}", shorter(token.getAccount()), token.getAccountMemo());
        ...
        if (asset == null ||
                !asset.getWithdraw().getEnabled() ||
                !asset.getSep24Enabled()
        ) {
        ...
        // Validate amount
        if (strAmount != null) {
            if (new BigDecimal(strAmount).compareTo(new BigDecimal(asset.getWithdraw().getMinAmount())) < 0
                    || new BigDecimal(strAmount).compareTo(new BigDecimal(asset.getWithdraw().getMaxAmount())) > 0) {
        ...
        Sep24Transaction txn = new Sep24TransactionBuilder(txnStore)
                .transactionId(txnId)
        ...
        Log.infoF("Saved withdraw transaction. from={}, amountIn={}, amountOut={}.", shorter(txn.getFromAccount()), txn.getAmountIn(), txn.getAmountOut());
    ...
    public InteractiveTransactionResponse deposit(String fullRequestUrl, JwtToken token, Map<String, String> depositRequest) throws SepException, MalformedURLException, URISyntaxException {
        if (token == null) {
            throw new SepValidationException("missing token");
        }
        ...
        Log.infoF("Sep24.deposit. account={}, memo={}", shorter(token.getAccount()), token.getAccountMemo());
        ...
        if (strAmount != null) {
            if (new BigDecimal(strAmount).compareTo(new BigDecimal(asset.getDeposit().getMinAmount())) < 0
                    || new BigDecimal(strAmount).compareTo(new BigDecimal(asset.getDeposit().getMaxAmount())) > 0) {
                throw new SepValidationException(String.format("invalid amount: %s", strAmount));
            }
        }
        ...
        txnStore.save(txn);
        Log.infoF("Saved deposit transaction. to={}, amountIn={}, amountOut={}.", shorter(txn.getToAccount()), txn.getAmountIn(), txn.getAmountOut());
        Log.debug("Transaction details:", txn);
    ...
    public GetTransactionsResponse findTransactions(JwtToken token, GetTransactionsRequest txReq) throws SepException, MalformedURLException, URISyntaxException {
        if (token == null) {
            throw new SepValidationException("missing token");
        }
    ...
    public GetTransactionResponse findTransaction(JwtToken token, GetTransactionRequest txReq) throws SepException, IOException, URISyntaxException {
        if (token == null) {
            throw new SepValidationException("missing token");
        }
    ...
    TransactionResponse fromTxn(Sep24Transaction txn) throws MalformedURLException, URISyntaxException, SepException {
    ...
    Memo makeMemo(String memo, MemoType memoType) throws SepException {
        try {
            ...
            throw new SepValidationException(String.format("Invalid memo %s of type:%s", memo, memoType), nfex);
        }
    }
    ...
    String constructInteractiveUrl(
            ...
            String assetCode,
            String amount,
            String txnId) throws URISyntaxException, MalformedURLException {
