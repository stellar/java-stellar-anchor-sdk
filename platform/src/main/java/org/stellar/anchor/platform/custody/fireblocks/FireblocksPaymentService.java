...
public class FireblocksPaymentService implements PaymentService {
  ...
  private static final String CREATE_NEW_TRANSACTION_PAYMENT_URL = "/transactions";
  ...
  @Retryable(
      ...
      maxAttemptsExpression = "${custody.fireblocks.max_attempts}",
      backoff = @Backoff(delayExpression = "${custody.fireblocks.delay}"),
      exceptionExpression =
          "#root instanceof T(org.stellar.anchor.api.exception.FireblocksException) AND"
              + "(#root.statusCode == 400 OR #root.statusCode == 429)")
