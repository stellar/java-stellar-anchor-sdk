...
class NotifyAmountsUpdatedTest {
  ...
  fun test_handle_unsupportedStatus() {
    ...
    val ex = assertThrows<InvalidRequestException> { handler.handle(request) }
    assertEquals(
      "Action[notify_amounts_updated] is not supported. Status[incomplete], kind[null], protocol[24], funds received[true]",
      ex.message
    )
  }
  ...
  fun test_handle_transferNotReceived() {
    ...
    val ex = assertThrows<InvalidRequestException> { handler.handle(request) }
    assertEquals(
      "Action[notify_amounts_updated] is not supported. Status[pending_anchor], kind[null], protocol[24], funds received[false]",
      ex.message
    )
  }
