...
public class Sep10Service {
    ...
    public Sep10Service(AppConfig appConfig,
    ...
    public ChallengeResponse createChallenge(ChallengeRequest challengeRequest) throws SepException {
        ...
        if (sep10Config.isClientAttributionRequired()) {
            ...
            if (denyList != null && denyList.size() > 0 && denyList.contains(challengeRequest.getClientDomain())) {
                infoF("ALERT: client domain provided is in configured deny list - {} ", challengeRequest.getClientDomain());
                throw new SepValidationException("unable to process.");
            }

            List<String> allowList = sep10Config.getClientAttributionAllowList();
            if (allowList != null && allowList.size() > 0 && !allowList.contains(challengeRequest.getClientDomain())) {
                infoF("ALERT: client domain provided is not in configured allow list - {} ", challengeRequest.getClientDomain());
                throw new SepValidationException("unable to process");
            }
        }
        // Validate account
        ...
        try {
            ...
            Transaction txn = Sep10Challenge.newChallenge(
                    signer,
                    ...
                    (challengeRequest.getClientDomain() == null) ? "" : challengeRequest.getClientDomain(),
            ...
            return ChallengeResponse.of(
                    ...
                    appConfig.getStellarNetworkPassPhrase()
            );
        } catch (URISyntaxException e) {
            throw new SepException(String.format("Invalid HOST_URL [%s} is used.", appConfig.getHostUrl()));
    ...
    public ValidationResponse validateChallenge(ValidationRequest validationRequest) throws IOException, InvalidSep10ChallengeException, URISyntaxException, SepValidationException {
        ...
        return ValidationResponse.of(
                generateSep10Jwt(
                        validationRequest.getTransaction(),
                        clientDomain
                )
        );
    }

    public String validateChallenge(String challengeXdr) throws IOException, InvalidSep10ChallengeException, URISyntaxException {
        Log.info("Parse challenge string.");
        Sep10Challenge.ChallengeTransaction challenge = Sep10Challenge.readChallengeTransaction(
                challengeXdr,
                serverAccountId,
                new Network(appConfig.getStellarNetworkPassPhrase()),
        ...
        infoF("Challenge parsed. account={}, home_domain={}", shorter(challenge.getClientAccountId()), challenge.getMatchedHomeDomain());

        String clientDomain = null;
        Operation operation = Arrays.stream(challenge.getTransaction().getOperations()).filter(op ->
        ...
        try {
            ...
            infoF("Verifying challenge threshold. server_account={}, signers={}", shorter(serverAccountId), signers.size());
            ...
            if ((clientDomain != null && challenge.getTransaction().getSignatures().size() != 3) ||
                ...
                infoF("ALERT: Invalid SEP 10 challenge exception, there is more than one client signer on challenge transaction for an account that doesn't exist");
        ...
        Set<Sep10Challenge.Signer> signers = Arrays.stream(account.getSigners())
        ...
        infoF("Verifying challenge threshold. server_account={}, threshold={}, signers={}", shorter(serverAccountId), threshold, signers.size());
    ...
    String getClientAccountId(String clientDomain) throws SepException {
        ...
        try {
            ...
            throw new SepException(String.format("SIGNING_KEY %s is not a valid Stellar account Id.", clientSigningKey));
        } catch (IOException ioex) {
            throw new SepException(String.format("Unable to read from %s", url), ioex);
        }
    }

    String generateSep10Jwt(String challengeXdr, String clientDomain) throws InvalidSep10ChallengeException, IOException, URISyntaxException {
        Sep10Challenge.ChallengeTransaction challenge = Sep10Challenge.readChallengeTransaction(
                challengeXdr,
                serverAccountId,
                new Network(appConfig.getStellarNetworkPassPhrase()),
