...
| app_config.app.hostUrl  | URL of the Anchor Platform SEP Service   | y  | N/A |
| app_config.app.jwtSecretKey | web encryption key | y | ${JWT_SECRET_KEY} | N/A | 
| app_config.app.stellarNetwork | TESTNET OR PUBNET| n | TESTNET |
| app_config.app.logLevel | TRACE,DEBUG,INFO,WARN,ERROR,FATAL | n | INFO |
| anchor.data_access.type  | database access type  | yes  | data-spring-jdbc  |
| anchor.data_access.setttings  | values config root-level key for jdbc config  | yes | data-spring-jdbc-sqlite  |
| app_config.app.integration_auth.auth_type | Anchor Platform to Anchor Backend Authentication type JWT_TOKEN, API_KEY or NONE | n | NONE |
| app_config.app.integration_auth.platformToAnchorSecret | secret value | n | ${PLATFORM_TO_ANCHOR_SECRET} |
| app_config.app.integration_auth.anchorToPlatformSecret | secret value | n | ${ANCHOR_TO_PLATFORM_SECRET} |
| app_config.app.integration_auth.expirationMilliseconds | integration auth credential expiration ms | n | 30000 |
| app_config.app.anchor_callback | endpoint to retrieve unique address & memo for sep 31 post /transaction  | n | N/A  |
...
| events.enabled  | sep1 true if event service enabled | yes | true |
| events.publisherType  | kafka|sqs  | no  | kafka  |
...
| metrics.enabled  | sep1 true if event service enabled | yes | true |
| metrics.extras  | kafka|sqs  | no  | kafka  |
